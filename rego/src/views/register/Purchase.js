		...
		<Theme>
			<Grid container direction="row" spacing={0}>
				...
				>
					<BookingOrderTotal />
					<Hidden smUp>
						<Typography className={classNames(classes.red)}>{error}</Typography>
						<Button
							className={classNames(classes.buttonRoot, classes.checkoutButton)}
							onClick={checkoutButton}
						>
							Checkout
						</Button>
...
export class Purchasea extends Component {
	constructor(props) {
		super(props);

		this.updateUser = this.updateUser.bind(this);
		this.checkoutButton = this.checkoutButton.bind(this);
		this.addTicketButton = this.addTicketButton.bind(this);
	...
	componentDidMount() {
		if (localStorage.getItem("MMT_BOOKING_UID")) {
			this.props.getBooking(localStorage.getItem("MMT_BOOKING_UID"));
		} else {
			this.props.home();
		}
	}
